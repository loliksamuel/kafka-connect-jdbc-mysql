...
ksql>CREATE TABLE accountGroupByTable  (last_name string, COUNT bigint) WITH (kafka_topic='mysql-08-accounts', value_format='JSON') ;
ksql>describe extended accountGroupByTable; --see the columns & how many massages
ksql>select * from accountGroupByTable;
...
Query OK, 1 row affected (0.00 sec)
verify after 1 sec, that u see it in the table
